    ...
    def fetch_and_save_stock_data(
        ...
        try:
            ...
            if not force_refresh and self.db.has_today_data(fetch_code, today):
                logger.info(f"{stock_name}({fetch_code}) 今日数据已存在，跳过获取（断点续传）")
                return True, None

            # 从数据源获取数据
            logger.info(f"{stock_name}({fetch_code}) 开始从数据源获取数据...")
            ...
            logger.info(f"{stock_name}({fetch_code}) 数据保存成功（来源: {source_name}，新增 {saved_count} 条）")
