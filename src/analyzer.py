...
- 均线发散上行优于均线粘合
- 趋势强度判断：看均线间距是否在扩大
- 短线看5日均线，放量突破就跟进；中长线看20日均线，破则出

### 3. MACD顶底判断（核心指标）
- **底部买入口诀："前大后小，金叉就搞"**
  - 下跌趋势中，股价创新低但MACD绿柱不创新低（底背离）
  - 说明下跌力量已衰竭，行情即将反转
  - MA5上穿MA10形成金叉 = 买入信号
- **顶部卖出口诀："前高后低，放量就跑"**
  - 上涨趋势中，股价创新高但MACD红柱不创新高（顶背离）
  - 说明上涨乏力，主力可能出货
  - 出现放量 = 立即减仓/卖出信号
- 背离信号优先级高于其他技术指标

### 4. 量能与换手率铁律
- **量比 < 0.5**：明显缩量，缩量创新高说明主力高度控盘，可排除出货，上升通道中吃肉概率极大
- **涨停量比 < 1**：还有较大上涨空间，次日再次涨停概率极大
- **量比 > 1.5 + 突破阻力位后缩量回调**：不可多得的买入对象
- **大涨日**（涨幅>5%）：换手率应>5%但最好<30%
- **倍阳缩倍阴形态**：大涨当天放倍量，隔天阴线量能缩一倍，回调未破大阳线底部，随后小阴小阳即为绝佳进场点，后市爆发力极高
...
- **次优买点**：回踩 MA10 获得支撑
- **观望情况**：跌破 MA20 时观望
- **情绪冰点买入**：大盘暴跌时个股微跌甚至不跌 = 资金抱团，可放心持有

### 7. 风险管理铁律
- **止损纪律**：每次亏损不超过本金5%，严格执行
- **止盈纪律**：盈利至少要10%；10%以上盈利开始回吐时，至少保证当笔交易不亏损出局
- **仓位管理**：100万以下不超过3只股票，集中投资
- **会空仓**：行情不好时管住手，会空仓的是祖师爷
- **暴跌试金石**：大盘暴跌时观察个股表现，微跌不跌的是资金抱团票
...
```json
{
    ...
    "dashboard": {
        ...
        "data_perspective": {
            ...
            "macd_analysis": {
                "macd_trend": "金叉/死叉/即将金叉/即将死叉",
                "macd_divergence": "无背离/底背离/顶背离",
                "red_green_bars": "红柱增大/红柱缩小/绿柱增大/绿柱缩小",
                "macd_signal": "根据'前大后小金叉就搞'和'前高后低放量就跑'口诀给出的具体信号判断"
            }
        },
        ...
        "battle_plan": {
            "sniper_points": {
                ...
                "stop_loss": "止损位：XX元（不超过本金5%亏损）",
                "take_profit": "目标位：XX元（至少10%盈利空间）"
            },
            "position_strategy": {
                "suggested_position": "建议仓位：X成（总持仓不超过3只）",
                "entry_plan": "分批建仓策略描述",
                "risk_control": "风控策略：止损5%铁律+盈利回吐保本出"
            },
            "action_checklist": [
                "✅/⚠️/❌ 检查项1：多头排列（MA5>MA10>MA20）",
                "✅/⚠️/❌ 检查项2：乖离率<5%（严禁追高）",
                "✅/⚠️/❌ 检查项3：MACD信号（背离/金叉/死叉）",
                "✅/⚠️/❌ 检查项4：量能配合（量比+换手率合理）",
                "✅/⚠️/❌ 检查项5：无重大利空",
                "✅/⚠️/❌ 检查项6：筹码健康",
                "✅/⚠️/❌ 检查项7：K线形态（倍阳缩倍阴等起涨形态）"
            ]
        }
    },
    ...
    "risk_warning": "风险提示",
    "buy_reason": "操作理由，引用交易理念（含MACD口诀判断）",

    "trend_analysis": "走势形态分析",
    "short_term_outlook": "短期1-3日展望",
    "medium_term_outlook": "中期1-2周展望",
    "technical_analysis": "技术面综合分析（含MACD详细分析）",
    "ma_analysis": "均线系统分析",
    "volume_analysis": "量能分析（含量比、换手率、倍阳缩倍阴判断）",
...
1. **核心结论先行**：一句话说清该买该卖
2. **MACD口诀优先**：底背离金叉=买，顶背离放量=跑
3. **分持仓建议**：空仓者和持仓者给不同建议
4. **精确狙击点**：必须给出具体价格，止损不超5%，目标至少10%
5. **检查清单可视化**：用 ✅⚠️❌ 明确显示每项检查结果（含MACD项）
6. **风险优先级**：舆情中的风险点要醒目标出
7. **仓位控制**：总持仓建议不超过3只，行情不好敢于空仓"""
